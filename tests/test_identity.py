...
class Identity(unittest.TestCase):
    ...
    def trip(self, **fields):
        """An earlier call carrying whatever identifiers it carried. History is the register (§3.1),
        and a draft is part of it: the caller really did say those values."""
    ...
    def at_sea(self, **fields):
        """An accepted log on: the vessel is out. Needs the mandatory set (ACC-1)."""
        i = self.trip(**fields)
        for field, value in (('pob', '2'), ('departurePoint', 'Marina'), ('destination', 'Facing Island')):
            if not L.get(self.cur, i)[field]:
                L.set_field(self.cur, i, field, value, 'alice', self.clock)
        L.set_field(self.cur, i, 'etaDay', 'today', 'alice', self.clock)
        L.set_field(self.cur, i, 'eta', '2300', 'alice', self.clock)
        L.accept(self.cur, i, 'alice', self.clock)
        return i
    ...
    def test_two_identifiers_that_agree_are_verified(self):
        ...
        i = self.trip(registration='AB123Q', memberNumber='4471')
        got = self.verify(i)
    ...
    def test_one_identifier_is_never_enough(self):
        ...
        i = self.trip(registration='AB123Q')
        got = self.verify(i)
    ...
    def test_identifiers_pointing_at_different_boats_are_a_conflict(self):
        ...
        i = self.trip(registration='AB123Q', memberNumber='8802')
        got = self.verify(i)
    ...
    def test_a_member_with_several_boats_is_not_a_conflict(self):
        ...
        i = self.trip(registration='CD456R', memberNumber='4471')
        got = self.verify(i)
        self.assertEqual(got['outcome'], 'verified', got['basis'])

    def test_an_unknown_second_identifier_is_partial_not_verified(self):
        self.trip(registration='AB123Q', memberNumber='4471')
        i = self.trip(registration='AB123Q', mobile='0400 000 000')
        got = self.verify(i)
    ...
    def test_the_same_value_twice_is_not_two_identifiers(self):                 # IDV-1
        self.trip(registration='AB123Q', memberNumber='4471')
        i = self.trip(registration='AB 123-Q', vesselName='AB123Q')
    ...
    def test_one_letter_wrong_offers_the_boat_without_claiming_a_match(self):
        self.trip(registration='AB123Q', memberNumber='4471')
        i = self.trip(registration='AB128Q', memberNumber='4471')   # 3 and 8 are not confusable
        got = self.verify(i)
        self.assertEqual(got['outcome'], 'partial', got['basis'])
        rego = [e for e in got['evidence'] if e['kind'] == 'registration'][0]
        self.assertIsNone(rego['match'])
        i = self.trip(registration='AB1230', memberNumber='4471')   # Q is not in a group either
    ...
    def test_a_confusable_letter_suggests_the_right_boat(self):
        ...
        i = self.trip(registration='DX77', memberNumber='4471')     # B and D rhyme on radio
        got = self.verify(i)
        rego = [e for e in got['evidence'] if e['kind'] == 'registration'][0]
    ...
    def test_applying_never_overwrites_what_the_caller_just_said(self):
        self.trip(registration='AB123Q', memberNumber='4471', mobile='0412 345 678')
        i = self.trip(registration='AB123Q', mobile='0499 999 999')
